...
async function run() {
    try {
        ...
        const { data: sessions, error: sErr } = await supabase.from('whatsapp_sessions').select('brand_key').eq('status', 'CONNECTED');
        if (sErr) throw sErr;
        ...
        if (brands.length === 0) {
            ...
            const { data: aliases } = await supabase.from('ai_brand_aliases').select('brand_key');
        ...
        // Pentru fiecare brand, interogam daca are deja surse la categoria "servicii". Stergem pe cele globale vechi de pe ele daca s-au copiat gresit?
        // Actually, the user asked to put the prices for EACH QR. So we just insert the atomic sources.
        
        for (const brand of brands) {
            console.log(`\nProcesare Brand [${brand}]:`);
            
            // Verificam sa nu dublam prelungind lista
            const { data: existing } = await supabase.from('vertex_sources')
                                        .select('id, title')
                                        .eq('brand_key', brand)
            ...
            // Inseram cele 7 pachete nucleare
            ...
            const { error: insErr } = await supabase.from('vertex_sources').insert(insertPayload);
            if (insErr) {
