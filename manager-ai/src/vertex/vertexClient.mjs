...
const VERTEX_MODEL = process.env.GEMINI_MODEL || 'gemini-2.0-flash-lite';
...
export async function processWithVertexAI(phoneE164, userMessageText, options = {}) {
    ...
    for (let i = 0; i < MAX_TOOL_CALLS; i++) {
        ...
        for (const fn of currentIterCalls) {
            ...
            if (name === 'noteaza_petrecere' || name === 'NoteazaPetrecereEventDetails') {
                ...
                // Only block if BOTH role AND character are identical (and character is NON-EMPTY)
                // If character is empty, we can't distinguish them — allow through, let JSONB merge handle it
                if (currentChar) {
                    for (const prev of toolCalls) {
                        if (prev.name === 'noteaza_petrecere' || prev.name === 'NoteazaPetrecereEventDetails') {
                            const prevRole = (prev.args.role_title || '').toLowerCase().trim();
                            const prevChar = getChar(prev.args.event_details);
                            if (currentRole && prevRole && currentRole === prevRole && prevChar === currentChar) {
                                isDuplicate = true;
                                break;
                            }
                        }
